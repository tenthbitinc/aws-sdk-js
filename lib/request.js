...
AWS.Request = inherit({
  ...
  eachPage: function eachPage(callback) {
    function wrappedCallback(response) {
      var result = callback(response.error, response.data);
  ...
  eachItem: function eachItem(callback) {
    ...
    function wrappedCallback(err, data) {
      ...
      var config = request.service.paginationConfig(request.operation);
      AWS.util.arrayEach(data[config.resultKey], function(item) {
        callback(null, item);
      });
    }
...
AWS.Response = inherit({
  ...
  nextPage: function nextPage(callback) {
    ...
    var config = this.request.service.paginationConfig(operation, true);
    var params = AWS.util.copy(this.request.params);
    if (!this._paginatedOutputTokenData()) {
      return callback ? callback(null, null) : null;
    } else {
      if (AWS.util.isType(config.inputToken, Array)) {
        for (var i = 0; i < config.inputToken.length; i++) {
          params[config.inputToken[i]] = this.data[config.outputToken[i]];
        }
      } else {
        params[config.inputToken] = this._paginatedOutputTokenData();
      }
  ...
  hasNextPage: function hasNextPage() {
    return this._hasNextPage(false);
  },

  _hasNextPage: function _hasNextPage(willThrow) {
    if (this.error) return false;

    var operation = this.request.operation;
    var config = this.request.service.paginationConfig(operation, willThrow);
    if (!config) return false;
    else if (config.moreResults) {
      return this.data[config.moreResults];
    } else {
      return this._paginatedOutputTokenData() ? true : false;
    }
  },

  _paginatedOutputTokenData: function _paginatedOutputTokenData() {
    if (this.paginatedOutputToken) return this.paginatedOutputToken;
    this.paginatedOutputToken = null;

    var config = this.request.service.paginationConfig(this.request.operation);
    var tokens = config.outputToken;
    if (typeof tokens === 'string') tokens = tokens.split(' or ');
    AWS.util.arrayEach.call(this, tokens, function (token) {
      var obj = this.data;
      var foundMatch = false;
      AWS.util.arrayEach.call(this, token.split('.'), function (part) {
        var match = part.match(/^(.+?)(\[-1\])?$/);
        obj = obj[match[1]];
        if (obj && match[2]) obj = obj[obj.length - 1];
        if (!obj) return AWS.util.abort;
      });

      if (obj) {
        this.paginatedOutputToken = obj;
        return AWS.util.abort;
      }
    });

    return this.paginatedOutputToken;
  }
