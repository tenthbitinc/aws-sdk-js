...
AWS.Request = inherit({
  ...
  eachItem: function eachItem(callback) {
    function wrappedCallback(err, data) {
      ...
      var config = this.request.service.paginationConfig(request.operation);
      var results = AWS.util.jamespath.query(config.resultKey, data);
      AWS.util.arrayEach(results, function(item) { callback(null, item); });
    }
...
AWS.Response = inherit({
  ...
  cacheNextPageTokens: function cacheNextPageTokens() {
    ...
    if (config.moreResults) {
      if (!AWS.util.jamespath.query(config.moreResults, this.data)) {
    ...
    AWS.util.arrayEach.call(this, exprs, function (expr) {
      var output = AWS.util.jamespath.query(expr, this.data);
      if (output) {
